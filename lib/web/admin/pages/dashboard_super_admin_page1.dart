    ...
    child: InputDecorator(
      ...
      child: Row(
        children: [
          ...
          const Icon(Icons.campaign_rounded, color: redColor, size: 22),
          const SizedBox(width: 2),
          const Icon(Icons.keyboard_arrow_down_rounded, color: redColor, size: 26),
        ],
      ),
...
Widget _buildAdvertiserDetailPanel() {
  ...
  final status = _cleanText(advertiser['status'] ?? 'Non renseigné');
  final visibility = _cleanText(advertiser['visibility'] ?? 'Non renseignée');
  final diffusion = _cleanText(advertiser['diffusion'] ?? 'Non renseignée');
  final price = _toDouble(advertiser['priceExclTax'] ?? advertiser['totalPriceExclTax']);
  final bannerUrl = _cleanText(advertiser['bannerUrl'] ?? advertiser['imageUrl']);
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.campaign_rounded, color: redColor, size: 28),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
                    companyName,
            ...
            _spotInfoLine('Statut', status),
            _spotInfoLine('Visibilité', visibility),
            _spotInfoLine('Diffusion', diffusion),
            _spotInfoLine('Début', _formatDate(advertiser['startDate'])),
            _spotInfoLine('Fin', _formatDate(advertiser['endDate'])),
...
void _centerOnFirstCurrentResult() async {
  // Recherche SPHOT
  final spotResults = _latestSpotDocs.where((doc) {
    final data = doc.data();
    final lat = _toDouble(data['sphotLat']);
    final lng = _toDouble(data['sphotLng']);

    return lat != 0 && lng != 0 && _matchesSearch(data);
  }).toList();

  if (spotResults.isNotEmpty) {
    final data = spotResults.first.data();
    ...
    _mapController.move(
      LatLng(_toDouble(data['sphotLat']), _toDouble(data['sphotLng'])),
      16,
    );
    return;
  }
  ...
  // Recherche ADMIN
  final adminResults = _latestAdminDocs.where((doc) {
    return _matchesAdminSearch(doc.data());
  }).toList();

  if (adminResults.isNotEmpty) {
    final data = adminResults.first.data();
    final territoire = Map<String, dynamic>.from(data['territoire'] ?? {});
    ...
    _mapController.move(
      LatLng(
        _toDouble(territoire['villeLat']),
        _toDouble(territoire['villeLng']),
      ),
      18,
    );
    return;
  ...
  // Recherche ANNONCEUR directement dans Firebase
  final adsSnapshot =
      await FirebaseFirestore.instance.collection('adRequests').get();

  final advertiserResults = adsSnapshot.docs.where((doc) {
    final data = doc.data();
  ...
  if (advertiserResults.isEmpty) return;

  final adData = {
    ...advertiserResults.first.data(),
    'id': advertiserResults.first.id,
  };

  setState(() {
    _selectedSpot = null;
    _selectedAdmin = null;
    _selectedAdvertiser = adData;
  });

  _mapController.move(
    LatLng(
      _toDouble(adData['centerLat']),
      _toDouble(adData['centerLng']),
    ),
    16,
  );
}
              ...
              return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, adsSnapshot) {
                  ...
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    ...
                    if (_visibleAdvertiserCount != validAdvertisers.length) {
                      setState(() {
                        _visibleAdvertiserCount = validAdvertisers.length;
                      });
                    }
                  });
