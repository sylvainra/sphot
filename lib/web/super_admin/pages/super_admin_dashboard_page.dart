...
class _SuperAdminDashboardPageState extends State<SuperAdminDashboardPage> {
        ...
        children: [
          SizedBox(
            ...
            child: Text(
              ...
              softWrap: wrapLabel,
              overflow: wrapLabel ? TextOverflow.clip : TextOverflow.visible,
          ...
          if (wrapLabel) const SizedBox(width: 12),
          Expanded(
            child: Text(
              value,
              style: const TextStyle(
  ...
  Widget _advertiserRequestInfoLine(String label, String value) {
    return LayoutBuilder(
      builder: (context, constraints) => _spotInfoLine(
        label,
        value,
        labelWidth: constraints.maxWidth * 0.38,
        bottomPadding: 8,
        wrapLabel: true,
      ),
    );
  ...
  Widget _buildRightPanel({required int visibleSpots}) {
    return Container(
      width: 360,
      decoration: BoxDecoration(
  ...
  Widget _buildAdminDetailPanel() {
    ...
    final responsableNom = _cleanText(
      profile['nomAffiche'] ?? admin['nomResponsable'] ?? admin['nom'],
    );

    final responsablePrenom = _cleanText(
      profile['prenomAffiche'] ?? admin['prenomResponsable'] ?? admin['prenom'],
    );
    ...
    final siren = _cleanText(structure['siren'] ?? admin['siren']);

    final ville = _cleanText(territoire['ville'] ?? admin['ville']);

    final adresse = _cleanText(territoire['adresse'] ?? admin['adresse']);

    final codePostal = _cleanText(
      territoire['codePostal'] ?? admin['codePostal'],
    );

    final departement = _cleanText(
      territoire['departement'] ?? admin['departement'],
    );

    final region = _cleanText(territoire['region'] ?? admin['region']);

    final pays = _cleanText(territoire['pays'] ?? admin['pays']);
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    icon: const Icon(Icons.close_rounded, color: adminColor),
                  ),
                ],
              ),
              const SizedBox(height: 10),
              ...
              _adminDetailSection(
                ...
                children: [
                  ...
                  _spotInfoLine(
                    'SIRET',
                    siret.isEmpty ? 'Non renseigné' : siret,
                  ),
                  _spotInfoLine(
                    'SIREN',
                    siren.isEmpty ? 'Non renseigné' : siren,
                  ),
                ],
              ),
              const SizedBox(height: 10),
              _adminDetailSection(
                title: 'TERRITOIRE',
                children: [
                  _spotInfoLine('Pays', pays.isEmpty ? 'Non renseigné' : pays),
                  _spotInfoLine(
                    'Région',
                    region.isEmpty ? 'Non renseignée' : region,
                  ),
                  _spotInfoLine(
                    'Département',
                    departement.isEmpty ? 'Non renseigné' : departement,
                  ),
                  _spotInfoLine(
                    'Adresse',
                  ...
                  _spotInfoLine(
                    'Code postal',
                    codePostal.isEmpty ? 'Non renseigné' : codePostal,
                  ),
                  _spotInfoLine(
                    'Ville',
  ...
  String _normalizeLegalTerminology(String input) {
    ...
    return value
        ...
        .replaceAll(
          'SPHOT PUBLICITAIRE SPHOT PUBLICITAIRE',
          'SPHOT PUBLICITAIRE',
        );
  }
    ...
    try {
      ...
      for (final entry in legalDocuments.entries) {
        ...
        for (final chapter in snapshot.docs) {
          ...
          final currentTitle =
              (data['title'] ?? data['titre'] ?? '').toString();
          final currentContent =
              (data['content'] ?? data['texte'] ?? '').toString();
                ...
                return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                  ...
                  builder: (context, adsSnapshot) {
                    ...
                    return Row(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        _buildRightPanel(visibleSpots: validSpots.length),
                        Expanded(
                          child: Stack(
                            children: [
                              FlutterMap(
